...
using robotis::deepdrive::sensors::SensorState;
...
: Sensors(nh),
  ...
  sonar_(sonar)
{
  pub_ = nh->create_publisher<deepdrive_msgs::msg::SensorState>(topic_name, this->qos_);
...
{
  auto msg = std::make_unique<deepdrive_msgs::msg::SensorState>();
