#include "deepdrive_node/motors.hpp"

#include <memory>
#include <string>
#include <utility>

using deepdrive::Motors;
...
: nh_(nh),
  ...
  publish_tf_(false)
{
  RCLCPP_INFO(nh_->get_logger(), "Init Motors");

  auto qos = rclcpp::QoS(rclcpp::KeepLast(10));
  odom_pub_ = nh_->create_publisher<nav_msgs::msg::Odometry>("odom", qos);

  cmd_broadcaster_ = std::make_unique<std_msgs::msg::Int64MultiArray>(nh_);

  wheel_encoders_sub_ = nh_->create_subscription<sensor_msgs::msg::JointState>(
    "joint_states",
    qos,
    std::bind(&Motors::joint_state_callback, this, std::placeholders::_1));

}

void Motors::joint_state_callback(const sensor_msgs::msg::JointState::SharedPtr joint_state_msg)
{
...
void Motors::publish(const rclcpp::Time & now)
{
  auto odom_msg = std::make_unique<nav_msgs::msg::Odometry>();

  odom_msg->header.frame_id = frame_id_of_motors_;
  odom_msg->child_frame_id = child_frame_id_of_motors_;
  ...
  odom_tf.header.frame_id = frame_id_of_motors_;
  odom_tf.child_frame_id = child_frame_id_of_motors_;
  odom_tf.header.stamp = now;

  odom_pub_->publish(std::move(odom_msg));

  if (publish_tf_) {
    cmd_broadcaster_->sendTransform(odom_tf);
  }
}

void Motors::update_joint_state(
  const std::shared_ptr<sensor_msgs::msg::JointState const> & joint_state)
{
  static std::array<double, 4> last_joint_positions = {0.0f, 0.0f};
...
bool Motors::calculate_odometry(const rclcpp::Duration & duration)
{
